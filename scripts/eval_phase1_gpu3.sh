...
echo "===== Phase 1 Eval (GPU3): Qwen3-1.7B × 5 benchmarks ====="
...
for SEED in "${SEEDS[@]}"; do
    for METHOD in "${METHODS[@]}"; do
        ...
        if [ -f "$OUT_FILE" ]; then
            echo "[GPU3] SKIP eval ${METHOD}/seed_${SEED} (benchmark_rouge.json exists)"
            continue
        fi

        # Skip if checkpoint missing
        if [ ! -f "$CKPT" ]; then
            echo "[GPU3] SKIP eval ${METHOD}/seed_${SEED} (no checkpoint)"
            continue
        fi

        echo "[GPU3] >>> eval ${METHOD}/seed_${SEED}"
