#!/bin/bash
# =============================================================================
# Phase 2 (half): Task-specific training on GPU 1
#   Student: Qwen3-0.6B
...
DATA_DIR="data"
LOG_DIR="logs/gpu1_phase2_${STUDENT_TAG}"
...
run_one() {
    ...
    if [ -f "$CKPT" ]; then
        echo "[GPU1] SKIP ${DATASET}/${METHOD}/seed_${SEED} (checkpoint exists)"
        return 0
    fi

    echo "[GPU1] >>> ${DATASET}/${METHOD}/seed_${SEED}"
...
echo "===== Phase 2 (GPU1): ${STUDENT_TAG} × 3 datasets × 8 methods × 3 seeds ====="
...
echo "===== GPU1 Phase 2 training done ====="
